...
public class Control {
    ...
    public static void main(String[] args) throws IOException {
        File dir = new File("/home/sri/p/font/FONT");
        File oDir = new File("/home/sri/p/font/font1CSV");
...
//    }
    ...
    Control(CharImage image) throws IOException {
        // Given image, convert to black and white
        img = new CharImage(image.width, image.height);
        pChar = new ArrayList<PixelCharacter>();
        for(int i = 0; i < img.width; i++) {
            for (int j = 0; j < img.height; j++) {
                if (image.isBelowThreshold(i, j))
                    img.setColor(i, j, CharImage.black);
                else
                    img.setColor(i, j, CharImage.white);
            }
        }
    }
